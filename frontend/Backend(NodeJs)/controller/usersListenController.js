        ...
        const sermonDetails = sermons.map(sermon => {
            ...
            const listeners = usersListens
                ...
                .map(userListen => {
                    ...
                    const clickedJesus = jesusClicks.some(jc => jc.userId.toString() === userListen.userId.toString() && jc.sermonId.toString() === sermon._id.toString());
...
export const getLiveUsersListen = async (req, res) => {
    try {
        ...
        const adminStaffIdsLive = [...new Set(sermonsLive.map(sermon => sermon.adminStaffUserId?.toString()))];
        ...
        const sermonDetailsLive = sermonsLive.map(sermon => {
            const admin = adminStaffUsers.find(user => user._id.toString() === sermon.adminStaffUserId.toString());
            const church = churches.find(ch => ch && sermon.churchId && ch._id.toString() === sermon.churchId.toString());

            const sermonListeners = usersListensLive.filter(userListen => userListen.sermonId.toString() === sermon._id.toString());
            ...
            const listeners = sermonListeners.map(userListen => {
                const user = listeningUsers.find(user => user._id.toString() === userListen.userId.toString());
                const clickedJesus = jesusClicksLive.some(jc => jc.userId.toString() === userListen.userId.toString() && jc.sermonId.toString() === sermon._id.toString());
