...
const DashDefault = () => {
  ...
  // useEffect(() => {
  //   fetchData(0, `${apiBaseUrl}/church/count`); // Fetch total churches
...
useEffect(() => {
  ...
  const userSpecificData = [
    { title: 'Attended Sermons', icon: <FaChartLine /> }, // Icon for attended sermon
    { title: 'Live Sermons', icon: <FaVideo /> }, // Icon for live sermon
    { title: 'Donations', icon: <FaMoneyBillAlt /> }, // Icon for donation
  ];
  ...
  const userSpecificDataStaff = [
    ...
    // { title: 'Total Staff', icon: <FaUsers />, count: 0 }, 
    // { title: 'Attended Sermons', icon: <FaChartLine /> }, // Icon for attended sermon
    { title: 'Total User', icon: <FaUserCheck />}, // Icon for attended sermon
  ...
  if (parseInt(userType) === 3) {
    return (
      <React.Fragment>
        <Row>
          {console.log(userSpecificDataStaff)}
          {userSpecificDataStaff.map((data, index) => (
            <Col key={index} xl={6} xxl={4}>
              <Card>
                <Card.Body>
                  <div className="d-flex justify-content-between align-items-center mb-4">
