...
const validationSchema = Yup.object().shape({
  ...
  //     // Basic email regex that finds the position of .com, .net, etc.
  //     const domainMatch = trimmedEmail.match(/\.[a-z]{2,}$/i);
  //     if (!domainMatch) return false;

  //     const domainEndIndex = domainMatch.index + domainMatch[0].length;
  //     const beforeDomainEnd = trimmedEmail.substring(0, domainEndIndex);

  //     // Check if the part before .com is a valid email
  //     const emailRegex = /^[^\s@]+@[^\s@]+\.[a-z]{2,}$/i;
  //     if (!emailRegex.test(beforeDomainEnd)) return false;

  //     // Allow only spaces after the domain
  //     const afterDomainEnd = trimmedEmail.substring(domainEndIndex);
  //     if (afterDomainEnd && !/^\s*$/.test(afterDomainEnd)) return false;
...
const SignupScreen = () => {
  ...
  const handleResetPasswordSubmit = async () => {
    ...
    try {
      setIsLoading(true); // Add a loading state to show a spinner/progress
      const response = await axios.post(`${apiBaseUrl}/user/resetPassword`, { email });
      setIsLoading(false);
      
      if (response.status === 200) {
        // We've received a successful response
        setShowCodeInput(true); // Show code input screen
        alert('A 4-digit reset code has been sent to your email. Please check your inbox.');
      } else {
  ...
  const handleCodeSubmit = async () => {
    if (!code || code.length !== 4 || !newPassword) {
      alert('Please enter a valid 4-digit code and new password');
      return;
    }
  ...
  //   if (field === 'email') {
  //     // Find the position of .com, .net, etc.
  //     const domainMatch = value.match(/\.[a-z]{2,}/i);
  //     if (domainMatch) {
  //       const domainEndIndex = domainMatch.index + domainMatch[0].length;
  //       const beforeDomain = value.substring(0, domainEndIndex);
  //       const afterDomain = value.substring(domainEndIndex);
        
  //       // Remove spaces before domain end, allow only spaces after
  //       const cleanedEmail = beforeDomain.replace(/\s/g, '') + afterDomain.replace(/[^\s]/g, '');
  //       setFormData(prev => ({
  //         ...prev,
  //         [field]: cleanedEmail
  //       }));
  //     } else {
  //       // If no domain found yet, remove all spaces
  //       setFormData(prev => ({
  //         ...prev,
  ...
  const handleInputChange = (field, value) => {
    if (field === 'email') {
      // Simply remove all spaces from email input
  ...
  const handleSubmit = async () => {
    ...
    if (isSignIn) {
      const loginData = {
        email: formData.email.trim().replace(/\s+/g, ''), // Remove all spaces for login
        password: formData.password,
      };
      try {
        const response = await axios.post(`${apiBaseUrl}/auth/login`, loginData);
      ...
      const registrationData = {
        ...
        lastName: formData.lastName,
        phone: formData.phone,
        email: formData.email.trim().replace(/\s+/g, ''), // Remove all spaces before saving
        suburb: formData.suburb,
        churchId: formData.churchId,
        password: formData.password,
        attendedBefore: formData.attendedBefore,
    ...
    >
      ...
      <Fade in={openTermsModal}>
        ...
        care for people.<br/><br/>
          ...
          <br/><br/>
          <Typography variant="body1">1. The information we collect</Typography><br/>
          ...
          events <br/><br/><Typography variant="body1">Sensitive/Special Category Personal Information.</Typography><br/><br/>
          ...
          <Typography variant="body1">• prayer requests.</Typography><br/>
          ...
          <Typography variant="body1">2. How we use your information</Typography><br/>  
          ...
          <Typography variant="body1"> 3. Access to your information </Typography><br/>
          ...
          <Typography variant="body1"> 4. Security </Typography><br/>
          ...
          information.<br/><br/>
          <Typography variant="body1">  5. Changes to this Privacy Policy</Typography><br/>
          ...
          <Typography variant="body1">  6. Contacting us</Typography><br/>
          ...
          SA 5075, AUSTRALIA. <br/><br/>

          {/* 3 - When someone else is signing for the first time insert a check box with the following text: I accept Futures Church will use this information in line with their Privacy Policy". Add the Privacy policy mentioned above next to the check box accept. */}
        ...
        {isSignIn && !showForgotPassword ? (
          ...
          >
            ...
            <Typography style={{ fontSize: '14px', color: '#666', lineHeight: 1.5 }}>
              Please enter your email address. We'll send you a 4-digit code to reset your password.
            </Typography>
            <TextField
              fullWidth
              label="Email Address"
