...
const JoinLiveSermon = () => {
  ...
  const handleLanguageChange = async (selectedOption) => {
    ...
    // If sermon is active, handle stream changes
    if (isSermonActive) {
      try {
        ...
        // Then connect appropriate streams based on selected language
        if (selectedOption.value === 'en') {
          // For English, just connect transcription stream
          connectEventSource();
        } else {
          // For other languages, connect both streams
          connectEventSource();
          // Short delay to ensure transcription stream is established
          await new Promise(resolve => setTimeout(resolve, 100));
          connectTranslationStream(selectedOption.value);
        }
